...
Test command line argument combinations.
  ...
  * path.separator=:
  ...
  [DRY-RUN] /*/bin/java -Djava.class.path=/* -Xmx64m org.scijava.parsington.Main --system (glob)
  ...
  [DRY-RUN] /*/bin/java -Djava.awt.headless=true -Dapple.awt.UIElement=true -Djava.class.path=/* -Xmx64m org.scijava.parsington.Main --system (glob)

  $ ./parsy --dry-run --system --heap 58m
  [DRY-RUN] /*/bin/java -Xmx58m -Djava.class.path=/* org.scijava.parsington.Main --system (glob)

  $ ./parsy --dry-run --system --class-path /tmp/lions.jar:/tmp/tigers.jar
  [DRY-RUN] /*/bin/java -Djava.class.path=/*:/tmp/lions.jar:/tmp/tigers.jar -Xmx64m org.scijava.parsington.Main --system (glob)

  $ ./parsy --dry-run --system --jar-path /tmp/jars:/tmp/other-jars
  [DRY-RUN] /*/bin/java -Djava.class.path=/* -Xmx64m org.scijava.parsington.Main --system --jar-path /tmp/jars:/tmp/other-jars (glob)

  $ ./parsy --dry-run --system --debugger 8765,suspend
  [DRY-RUN] /*/bin/java -agentlib:jdwp=transport=dt_socket,server=y,address=localhost:8765,suspend -Djava.class.path=/* -Xmx64m org.scijava.parsington.Main --system (glob)
